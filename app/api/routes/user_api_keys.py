...
class CreateUserKeyRequest(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="Name/label for the key")
    description: Optional[str] = Field(None, max_length=500)
    vote_weight: int = Field(default=10, ge=1, le=100, description="Vote weight multiplier")
...
@router.post("", response_model=CreateUserKeyResponse, dependencies=[Depends(require_admin)])
async def create_user_key(request: CreateUserKeyRequest):
    ...
    logger.info(f"Admin created user API key: {key_data.get('key_prefix')}*** for '{request.name}'")
...
@router.get("/stats", response_model=UserKeyStatsResponse, dependencies=[Depends(require_admin)])
...
@router.get("/{key_id}", response_model=UserKeyInfo, dependencies=[Depends(require_admin)])
async def get_user_key(key_id: int):
    ...
    return UserKeyInfo(
        ...
        last_used_at=key_data["last_used_at"].isoformat() if key_data.get("last_used_at") else None,
        created_at=key_data["created_at"].isoformat() if key_data.get("created_at") else "",
        expires_at=key_data["expires_at"].isoformat() if key_data.get("expires_at") else None,
    )


@router.patch("/{key_id}", response_model=UserKeyInfo, dependencies=[Depends(require_admin)])
