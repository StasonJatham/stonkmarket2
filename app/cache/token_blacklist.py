...
async def blacklist_user_tokens(username: str, before: Optional[datetime] = None) -> int:
    """
    ...
    try:
        ...
        logger.info(f"All tokens for user '{username}' issued before {timestamp.isoformat()} are now invalid")
        return 1
