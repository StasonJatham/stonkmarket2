...
class DipMetrics:
    ...
    # Persistence
    persist_days: int  # Days dip condition has held
...
def compute_dip_series_multi_window(
    ...
    """
    return {
        w: compute_dip_series_windowed(close_prices, w)
        for w in windows
    }
...
def compute_dip_metrics(
    ...
    peak_price = float(np.max(close_prices[start_idx:]))
...
class IncrementalDipTracker:
    ...
    def update(self, price: float) -> Optional[float]:
        ...
        # Remove elements outside window
        while self._max_deque and self._max_deque[0][0] <= self._day_index - self.window:
