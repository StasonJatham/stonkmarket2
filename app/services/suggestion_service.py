...
FETCH_DELAY_SECONDS = 2.0  # Delay between fetches to avoid rate limits
BATCH_SIZE = 5  # Number of suggestions to fetch per batch run
...
async def fetch_suggestion_data(symbol: str) -> dict:
    ...
    try:
        ...
        # Get current price
        result["last_price"] = info.get("regularMarketPrice") or info.get("previousClose")
...
async def process_pending_suggestions() -> dict:
    ...
    for i, suggestion in enumerate(pending):
        ...
        try:
            ...
            if data["success"]:
                results["success"] += 1
                results["symbols"].append({"symbol": suggestion.symbol, "status": "success"})
            else:
                results["failed"] += 1
                results["symbols"].append({"symbol": suggestion.symbol, "status": "failed"})
                
        except Exception as e:
            logger.error(f"Error processing suggestion {suggestion.symbol}: {e}")
            results["failed"] += 1
            results["symbols"].append({"symbol": suggestion.symbol, "status": "error", "error": str(e)})
...
def suggest_stock(symbol: str, voter_identifier: str) -> tuple[bool, Optional[str], Optional[dict]]:
    ...
    with get_db() as conn:
        ...
        if existing:
            ...
            success, vote_error = suggestions_repo.add_vote(conn, symbol, voter_identifier)
            if not success:
        ...
        return [
            {
                ...
                "sector": s.sector,
                "industry": s.industry,
                "summary": s.summary[:200] + "..." if s.summary and len(s.summary) > 200 else s.summary,
