...
# Preferred model - use gpt-4o-mini for best cost/performance
PREFERRED_MODEL = "gpt-4o-mini"
FALLBACK_MODEL = "gpt-4o-mini"  # Same model as fallback

# Retry settings for structured output
MAX_RETRIES = 3
...
class DipRatingResponse(BaseModel):
    """Structured response for dip rating."""
    rating: Literal["strong_buy", "buy", "hold", "sell", "strong_sell"] = Field(
        description="The rating for this dip opportunity"
    )
    reasoning: str = Field(
        description="2-3 sentence explanation of the rating",
        max_length=500
    )
...
class TinderBioResponse(BaseModel):
    """Structured response for tinder bio generation."""
    bio: str = Field(
        description="A witty Tinder-style bio for the stock, 2-3 sentences max",
        max_length=300
    )
    ...
    try:
        ...
        if row and row["request_metadata"]:
            data = (
                ...
                else json.loads(row["request_metadata"])
            )
            cached_at = row["recorded_at"]

            # Check if cache is still valid (7 days)
            if cached_at and datetime.utcnow() - cached_at < timedelta(
                days=CACHE_TTL_DAYS
            ):
...
async def generate_dip_bio_realtime(
    ...
    """
    Generate a dip bio in real-time using structured output.
...
Current Price: ${current_price or 0:.2f}
All-Time High: ${ath_price or 0:.2f}
Dip from ATH: {dip_percentage or 0:.1f}%
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response = await client.beta.chat.completions.parse(
                ...
                response_format=TinderBioResponse,
                max_tokens=150,
                temperature=0.8,
            )
            ...
            if parsed:
                ...
                logger.info(
                    f"Generated structured bio for {symbol} using {model}, cost: ${cost:.4f}"
                )
                # Strip whitespace and any wrapping quotes
                bio = parsed.bio.strip().strip('"').strip("'").strip()
                return bio

        except Exception as e:
            logger.warning(f"Bio attempt {attempt + 1}/{MAX_RETRIES} failed for {symbol}: {e}")
            if attempt == MAX_RETRIES - 1:
                logger.error(f"All retries exhausted for bio {symbol}")
                return None
...
async def rate_dip_realtime(
    ...
    """
    Rate a dip in real-time using structured output.

    Returns:
        Rating result dict with 'rating', 'reasoning', 'confidence' or None if failed
    """
    client = await _get_client()
    if not client:
        return None
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            response = await client.beta.chat.completions.parse(
                ...
                response_format=DipRatingResponse,
                max_tokens=200,
                temperature=0.3,
            )
            ...
            if parsed:
                ...
                logger.info(
                    f"Generated structured rating for {symbol}: {parsed.rating}, cost: ${cost:.4f}"
                )
                return {
            ...
            if attempt == MAX_RETRIES - 1:
                logger.error(f"All retries exhausted for rating {symbol}")
                return None
...
async def create_batch_job(
    ...
    for req in requests:
        custom_id = req.get(
            "custom_id", f"{job_type.value}_{req.get('symbol', 'unknown')}"
        )
...
Why suggested: {req.get("reason", "No reason provided")}"""
    ...
    try:
        ...
        logger.info(
            f"Created batch job {batch.id} for {job_type.value} with {len(requests)} items using {model}"
        )
        return batch.id
