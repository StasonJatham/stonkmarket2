...
from app.database.models import StockSuggestion
...
def count_by_status(conn: sqlite3.Connection, status: str) -> int:
    ...
    row = conn.execute(
        "SELECT COUNT(*) as cnt FROM stock_suggestions WHERE status = ?", (status,)
    ).fetchone()
...
def get_by_symbol(conn: sqlite3.Connection, symbol: str) -> Optional[StockSuggestion]:
    ...
    row = conn.execute(
        "SELECT * FROM stock_suggestions WHERE symbol = ?", (symbol.upper(),)
    ).fetchone()
...
def get_by_id(
    ...
    row = conn.execute(
        "SELECT * FROM stock_suggestions WHERE id = ?", (suggestion_id,)
    ).fetchone()
    return StockSuggestion.from_row(row) if row else None
...
def list_suggestions(
    ...
    count_row = conn.execute(
        f"SELECT COUNT(*) as cnt FROM stock_suggestions {where_clause}", params
    ).fetchone()
...
def can_suggest(conn: sqlite3.Connection, symbol: str) -> tuple[bool, Optional[str]]:
    ...
    Returns:
        ...
        Note: Returns True for pending suggestions so votes can be added.
    """
    symbol = symbol.upper()
    ...
    if existing.status == "pending":
        # Allow voting on pending suggestions
        return True, None
    ...
    if existing.status == "removed":
        ...
        if existing.removed_at:
            ...
            if datetime.utcnow() < cooldown_end:
                ...
                return (
                    False,
                    f"This stock was previously reviewed. It can be suggested again in {days_left} days",
                )
...
def add_vote(
    ...
    if existing_vote:
        ...
        return (
            False,
            f"You can vote for this stock again in {days_left} day{'s' if days_left != 1 else ''}",
            False,
        )
...
def reject(
    conn: sqlite3.Connection, suggestion_id: int, reason: Optional[str] = None
) -> Optional[StockSuggestion]:
    """Reject a suggestion."""
    now = datetime.utcnow().isoformat()
    conn.execute(
